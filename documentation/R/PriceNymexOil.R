...
url <- "http://quotes.ino.com/exchanges/contracts.html?r=NYMEX_WS"
...
nymex <- data.frame(
  ...
  ,nymexOilFuturePrice=nymexFutures$Last
)

nymexHistory <- Quandl("CHRIS/CME_CL1", api_key="", collapse="monthly", start_date="1999-01-01")
...
nymex <- data.frame(
  month=nymex$month
  ,nymexOilPrice=coalesce(nymex$nymexCrudeHistory,nymex$nymexCrudeFuture)
)
...
futuresEndMonthPrice<-as.numeric(nymex$nymexOilPrice[NROW(nymex)])
...
nymex <- 
  bind_rows(
    ...
    ,data.frame(
      ...
      ,nymexOilPrice=additionalFuturesPrice
    )
  )
...
conn <- dbConnect(drv
                  ...
                  , host="localhost"
                  , port = 25432)
dbWriteTable(conn, c("stage", "strip_price_oil"), nymex, overwrite=TRUE, schema="stage",row.names = FALSE)

dbDisconnect(conn)


plot(nymex$nymexOilPrice~nymex$month,type="o", col="blue")
