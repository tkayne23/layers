...
FROM (
SELECT sq.*, lead(oil) OVER (ORDER BY monthson)-oil as slope
FROM (
SELECT prodMonth as monthson, coalesce(avg(oilactuals),0) as oil, count(distinct api) as wellcount 
from \"public\".\"cogcc_decline_oil\" 
where prodMonth <= 60
group by prodmonth 
) sq
) sq
...
fit <- best.fit(data$oil,data$monthson)
...
dfOutput <-
  merge(
    data.frame(
      ...
      ,oilActuals=as.numeric(data$oil)
    )
    ,data.frame(
...
plot(dfOutput$oilForecast~dfOutput$month
     , lty="solid"
     , type = "l"
     , col="blue"
)
lines(dfOutput$oilActuals~dfOutput$month)
